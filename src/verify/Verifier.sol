...
contract Verifier {
  ...
  function _recover(string memory headerJson, string memory payloadJson, bytes memory signature, string memory subject) internal view {
  ...
  function _verifyToken(string memory headerJson, string memory payloadJson, bytes memory signature, string memory subject) internal view {
  }

  function _parseHeader(string memory json) internal pure returns (string memory kid) {
    (uint exitCode, JsmnSolLib2.Token[] memory tokens, uint ntokens) = json.parse(20);
  ...
  function _parseToken(string memory json) internal pure returns (string memory aud, string memory nonce, string memory sub) {
    (uint exitCode, JsmnSolLib2.Token[] memory tokens, uint ntokens) = json.parse(40);
    require(exitCode == 0, "JSON parse failed");
  ...
  function _addKey(string memory kid, bytes memory modulus) internal { // will require onlyAdmin
    keys[kid] = modulus;
  }
}